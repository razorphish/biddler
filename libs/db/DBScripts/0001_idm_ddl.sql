...
CREATE TABLE BIDDLER_IDM_DB.ENV_INFO (
  ...
  ENV_NAMES     VARCHAR(512)  NOT NULL COMMENT 'Space delimited array of possible names for a given environment (ex. dev, development)',
  CREATD_DT       DATETIME NOT NULL, 
  CREATD_BY       VARCHAR(48) NULL DEFAULT 'SYSTEM', 
  LAST_UPDATD_DT  DATETIME NOT NULL, 
  LAST_UPDATD_BY  VARCHAR(48) NULL DEFAULT 'SYSTEM', 
...
CREATE TABLE BIDDLER_IDM_DB.SYS_ISSUER (
  ...
  RFRSH_TKN_TTL       INT NOT NULL COMMENT '(Time-To-Live) Maximum amount of time (seconds) REFRESH token has to live in application',
  HASH_ALGORITHM      VARCHAR(32) NOT NULL COMMENT 'Algorithm to use when generating tokens',
...
CREATE TABLE BIDDLER_IDM_DB.API_CLIENT (
  ...
  STUS_LKP_ID         VARCHAR(32) NOT NULL,
  CLIENT_AUD          VARCHAR(48) NOT NULL COMMENT 'Audience (aud) that this ID Token is intended for.',
	CLIENT_SUB          CHAR(36) NOT NULL COMMENT 'Subject Identifier (sub) on token. A locally unique and never reassigned identifier within the Issuer for the End-User, which is intended to be consumed by the Client. ',
  KEY_SECRET_HASH     VARCHAR(128) NOT NULL COMMENT 'Client secret key HASH for generating access tokens', 
  KEY_SALT            VARCHAR(255) NULL COMMENT 'Client secret key salt used to formulate access tokens',
  SCOPES              VARCHAR(2048) NOT NULL COMMENT 'Space-delimited array of scopes',
  ALLOWED_IPS         VARCHAR(1024) NULL DEFAULT '*' COMMENT 'Space delimited array of IP addresses and/or IP Address ranges.  If blank or ''*'' then all are allowed, otherwise ONLY listed IP addresses allowed',
  RESTRICTED_IPS      VARCHAR(1024) NULL DEFAULT '-' COMMENT 'Space delimited array of IP addresses and/or IP Address ranges that are restricted. If blank or ''-'' then there are no restrictions',  
  TKN_TTL             INT NOT NULL COMMENT '(Time-To-Live) Maximum amount of time (seconds) ACCESS token has to live in application',
  RFRSH_TKN_TTL       INT NOT NULL COMMENT '(Time-To-Live) Maximum amount of time (seconds) REFRESH token has to live in application',
  CREATD_DT           DATETIME NOT NULL,
	CREATD_BY           VARCHAR(48) NULL DEFAULT 'SYSTEM',
